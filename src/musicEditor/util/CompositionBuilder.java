...
public interface CompositionBuilder<T> {
   ...
   * @param timbre The instrument number (in the range [1, 128])
   * @param midiPitch The pitch (in the range [0, 127],
   *                  where 60 represents C4, the middle-C on a piano)
   * @param volume The volume (in the range [0, 127])
   * @return This builder
   */
  CompositionBuilder<T> addNote(int start, int end, int timbre, int midiPitch, int volume);
}
